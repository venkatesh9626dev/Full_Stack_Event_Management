...
class Participant_Validator_Class:
    ...
    def is_already_registered(self, event_id, user_id):
        ...
        if booking_data:
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="User has already registered for this event")
...
class Creator_Validator_Class:
    ...
    def is_merchant_id_exists(self, creator_id):
        ...
        if not creator_profile:
            raise HTTPException(status_code=403, detail="User profile is required to create an event.")
        
        merchant_id = creator_profile.merchant_id
        
        if not merchant_id:
             raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="User should enter their razorpay merchant id to create a paid events to receive money from participants")
...
class Events_Validator_Class:
    ...
    def validate_event_details(self, event_data : dict, creator_id : bytes):
        ...
        self.time_validator.check_event_datetime(event_start_datetime,event_end_datetime)
...
events_validator = Events_Validator_Class(event_dao=event_dao,time_validator=time_validator, creator_validator = creator_validator)